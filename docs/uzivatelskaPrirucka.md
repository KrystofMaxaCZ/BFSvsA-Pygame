...
* **Start:** Přepne uživatele do Scény 1.
* **Porovnání:** Přepne uživatele do Scény 2.
...
* **Reset:** Uživateli resetuje úplně celé pole – vymaže se nalezená cesta i všechny postavené zdi a mřížka se vrátí do původního stavu.
* **Reset algoritmus:** Toto tlačítko uživateli vymaže pouze nalezenou cestu z mřížky, ale postavené zdi nechá na místě. To dovoluje uživateli znovu upravit pole nebo zkusit jiný algoritmus na stejném bludišti.
* **Tlačítko Porovnání BFS vs A*:** Slouží uživateli k okamžitému přesunu na druhou scénu (Scene 2).
...
Po kliknutí na tlačítko ,,POROVNAT" se spustí oba algoritmy. Aplikace v obou mřížkách vyznačí výslednou nejkratší cestu, ale především ukáže proces, jakým algoritmy pole prohledávaly.
...
* **Tlačítko POROVNAT:** Tímto tlačítkem se spustí oba algoritmy najednou. V mřížkách se zobrazí cesty, čísla vzdáleností a pod mřížkami se zobrazí počty průchodů.
* **Reset:** Uživateli resetuje obě mřížky, podobně jak ve scéně 1.
* **Reset algoritmus:** Toto tlačítko uživateli vymaže pouze výsledky hledání. Podobná funkce jako ve scéně 1.
* **Klávesa ESC:** Slouží pro návrat uživatele zpět do hlavního menu.
